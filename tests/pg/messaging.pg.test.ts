...
describe.skipIf(!available)('PostgreSQL จริง — การรับประกันของระบบส่งข้อความ', () => {
  ...
  it('ส่งสำเร็จ → ล้างสถานะ "เคยถูกปฏิเสธ" และเก็บ meta_message_id', async () => {
    // ⚠️ ต้องให้ "เวลาที่สังเกต" เก่ากว่า "เวลาที่ลูกค้าทัก" อย่างชัดเจน
    //
    // 🔴 เดิมเขียน now() ทั้งสองบรรทัด ซึ่งทำให้เทสต์วูบวาบ (flaky ~2 ใน 5) :
    //    engine เทียบด้วย >= โดยตั้งใจ (ตัดสินใจแบบ fail closed — เชื่อ Meta ไว้ก่อน)
    //    ถ้าสองเวลาตกอยู่ในมิลลิวินาทีเดียวกัน ข้อสังเกตเก่าจะยังถือว่าใช้ได้
    //    แล้วระบบจะบล็อกการส่ง ทั้งที่เจตนาของเทสต์คือ "ลูกค้าทักกลับมาทีหลัง"
    //
