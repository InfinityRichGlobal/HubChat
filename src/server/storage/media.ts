 ...
 *          ส่วนการคุยกับ Storage จริง ๆ อยู่ที่ server/storage/supabase-storage.ts ที่เดียว
 *
 * 🔴 กฎเหล็กสองข้อของไฟล์นี้ :
 *
 *   1. ⚠️ ห้ามทำให้การรับข้อความของลูกค้าพังเด็ดขาด
 *      ถ้า Storage ล่ม / ลิงก์หมดอายุ → จดไว้แล้วเดินต่อ
...
export async function captureInboundMedia(input: MediaCaptureInput): Promise<CaptureSummary> {
  ...
  for (const target of targets) {
    ...
    try {
       ...
       *
       * 🔴 เคยเขียนแบบเดาจากข้อความแล้วพลาด : ตอนปลายทางตอบ 403 กลับมา
